...
describe("Controller", function() {
  ...
  it("Controller should be able to deploy order contract", async function() {
    ...
    const linkToken = await LinkToken.deploy("Fake Link", "fLINK");
    ...
    log(explanation("\nLet's deploy " + chalk.whiteBright("Fake Link") + ", " + chalk.whiteBright("Oya Token") + ", " + "and the " + chalk.whiteBright("Controller") + "..."));
    log(chalk`
      {whiteBright Fake Link}: ${linkToken.address}
    ...
    log(chalk`
      ...
      {magentaBright address link}: ${linkToken.address}
    `);
    await sleep(1200);
