...
class BuidlerNode extends events_1.default {
    ...
    async getLogs(filterParams) {
        ...
        const logs = [];
        for (let i = filterParams.fromBlock; i <= filterParams.toBlock; i = i.addn(1)) {
...
If you are using a wallet or dapp, try resetting your wallet's accounts.`);
    ...
    async _computeFilterParams(filterParams, isFilter) {
        if (filterParams.fromBlock === filter_1.LATEST_BLOCK ||
            filterParams.toBlock === filter_1.LATEST_BLOCK) {
            const block = await this.getLatestBlock();
            if (filterParams.fromBlock === filter_1.LATEST_BLOCK) {
                filterParams.fromBlock = new ethereumjs_util_1.BN(block.header.number);
            }
            if (!isFilter && filterParams.toBlock === filter_1.LATEST_BLOCK) {
                filterParams.toBlock = new ethereumjs_util_1.BN(block.header.number);
            }
        }
        return filterParams;
    }
